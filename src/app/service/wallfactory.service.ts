...
export class WallFactoryService {
  ...
  createWall(scene: any, wallType : EWallType, param1: number, param2: number) : any {
    ...
    if (wallType == EWallType.eInsideWallDoor) {
      ...
      wallmat.diffuseTexture = new this.BABYLON.Texture("http://i.imgur.com/2b1C7UH.jpg", scene);

	  var rearWall = this.BABYLON.MeshBuilder.ExtrudePolygon("east", { shape: rearWallData, depth: 0.1, faceUV: rearFaceUV,  }, scene);
	  ...
	  var frontWall =  this.BABYLON.MeshBuilder.ExtrudePolygon("west",{shape: frontWallData, depth: 0.15,holes: frontWindow1Holes, faceUV: frontFaceUV}, scene);
	  ...
	  var rightWall = this.BABYLON.MeshBuilder.ExtrudePolygon("north", { shape: rightWallData, depth: 0.1 , faceUV: rightFaceUV  }, scene );
	  ...
	  var leftWall = this.BABYLON.MeshBuilder.ExtrudePolygon("south",{ shape: leftWallData, depth: 0.1,	holes: sideWindowHoles, faceUV: leftFaceUV },scene);
	  ...
	  var floor = this.BABYLON.MeshBuilder.ExtrudePolygon("floor", {shape: firstfloorData, depth: 0.1, faceUV: floorFaceUV }, scene );
	  ...
	  var ceiling = this.BABYLON.MeshBuilder.ExtrudePolygon("ceiling", { shape: ceilingData, depth: 0.1 }, scene  );
      ...
      var wallmat = new this.BABYLON.StandardMaterial("wallmaterial", scene);
	  wallmat.diffuseColor = new this.BABYLON.Color3(240 / 255,223 / 255,203 / 255,scene	);
    }
  }
  ...
  windowMaker(width,height,frames,frameDepth,frameThickness): any {
    var windowShape = [
		...
		var window = this.BABYLON.MeshBuilder.ExtrudePolygon(
			"window",
  ...
  doorMaker(width, height, depth): any{
    var doorShape = [
		...
		panel[0] = [];
		panel[0].push(
			new this.BABYLON.Vector3(this.edgeThickness, 0, 2 * this.edgeThickness + panelBotHeight)
		);
		panel[0].push(
			new this.BABYLON.Vector3(
		...
		panel[1] = [];
		panel[1].push(new this.BABYLON.Vector3(this.edgeThickness, 0, this.edgeThickness));
		panel[1].push(
			new this.BABYLON.Vector3(this.edgeThickness + panelWidth, 0, this.edgeThickness)
		);
		panel[1].push(
			new this.BABYLON.Vector3(
				this.edgeThickness + panelWidth,
		...
		panel[1].push(
			new this.BABYLON.Vector3(this.edgeThickness, 0, this.edgeThickness + panelBotHeight)
		);
		var door = new this.BABYLON.MeshBuilder.ExtrudePolygon(
			"door",
			{ shape: doorShape, holes: panel, depth: depth }
		
		);
		door.rotation.x = -Math.PI / 2;
		var panelB = new this.BABYLON.MeshBuilder.CreateBox(
			"p1b",
		...
		var panelT = this.BABYLON.MeshBuilder.CreateBox(
			"p1t",
			{ width: panelWidth, height: panelTopHeight, depth: depth / 2 }
		
		);
		panelT.position.x = this.edgeThickness + panelWidth / 2;
		panelT.position.y = 2 * this.edgeThickness + panelBotHeight + panelTopHeight / 2;
